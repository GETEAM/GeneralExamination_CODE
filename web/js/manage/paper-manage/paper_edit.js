$(function(){
	...
	var paper_id = $('.paper-id').html();


	/*$.ajax({
	    type: 'GET',
	    url: '/manage/paper/QTJSON/' + paper_id,
	    dataType: 'json',
	    async: true,
	...
	//很变态的获取方式，主要是之前是通过ajax获取的，但是本次加载还有另外一个ajax请求，所以只能获取表单内容
	var paper = $('#papermanagebundle_paper_content').val();
	paper = JSON.parse(paper);

	//编辑的时候题型整个重新添加，主要是是在找不到更好的修改试题的方法了。。。。。
	for(var m in paper.groups){
		var groups = paper.groups[m];
		for(var i in groups.parts){ 
	    	var secObj = groups.parts[i].sections;
	    	for (var j in secObj) {
	    		secObj[j].items=[];
	    	}
	    }
				...
				React.createElement('div',{className: 'group'},
					...
					React.createElement('div',{className: 'group-title'},
						...
						React.createElement('label',null,
							'该group时长：',
							React.createElement('input',{type:'text',placeholder:'该group的时长，分钟',value:group.duration,onChange:this.handleGroupDuration})
						)
					),
	...
	var Part = React.createClass({displayName: 'Part',
		...
		render: function(){
			...
			var partSelfProperty = React.createElement('div',{className:'part-property'},
				React.createElement('label',null,
					'Part名称：',
					React.createElement('input',{type: 'text', placeholder: '输入part名称',value:part.name,onChange:self.handlePartName})
				),
				React.createElement('label',null,
					'流程性：',
					React.createElement('input',{type: 'checkbox', title: '该part是否是流程性试题',checked:part.flowable,onChange:self.handlePartShuffle})
				),
				React.createElement('label',null,
					'Part时长：',
					React.createElement('input',{type: 'text', placeholder: '输入part时长，分钟',value:part.duration,onChange:self.handlePartDuration})
				)
			);
				...
				React.createElement('div',{className:'section'},
					...
					React.createElement('div', {className:'section-title'},
						...
						React.createElement('div',{className:'section-property'},
							React.createElement('label',null,
								'direction',
								React.createElement('input',{type: 'text', placeholder: 'direction即题目',value:section.direction, onChange:self.handleDirection})
							)
						)
					),
	...
	$('.paper-finish-btn .complete-btn').click(function(){
		...
		//最终要出题的paper的内容
		console.log(paper);
		$('.paper-structure').val(JSON.stringify(paper));
